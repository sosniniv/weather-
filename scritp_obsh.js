...
function searchTable() {
    ...
    // Получаем все строки таблицы, кроме первой строки (с полями ввода)
...
// Привязка функции к кнопке "Найти"
...
function updateRecordCounter() {  
    ...
    document.getElementById('recordCounter').textContent = count;  
}  
...
// Переменные для хранения выбранных значений сортировки  
let selectedPosition = null;  
let selectedDepartment = null;  
let selectedStatus = null;  

// Функция для установки выбранной должности  
function setSortPosition(position) {  
    selectedPosition = position;  
}  

// Функция для установки выбранного подразделения  
function setSortDepartment(department) {  
    selectedDepartment = department;  
}  

// Функция для установки выбранного статуса  
function setSortStatus(status) {  
    selectedStatus = status;  
}  

// Функция для отображения или скрытия выбора должности  
function togglePositionSelection() {  
    const positionSelection = document.getElementById("positionSelection");  
    positionSelection.style.display = positionSelection.style.display === "none" ? "block" : "none";  
}  

// Функция для отображения или скрытия выбора подразделения  
function toggleDepartmentSelection() {  
    const departmentSelection = document.getElementById("departmentSelection");  
    departmentSelection.style.display = departmentSelection.style.display === "none" ? "block" : "none";  
}  

// Функция для отображения или скрытия выбора статуса  
function toggleStatusSelection() {  
    const statusSelection = document.getElementById("statusSelection");  
    statusSelection.style.display = statusSelection.style.display === "none" ? "block" : "none";  
}  

// Функция для отображения или скрытия выбора даты сортировки  
function toggleDateSelection() {  
    const dateSelection = document.getElementById("dateSelection");  
    dateSelection.style.display = dateSelection.style.display === "none" ? "block" : "none";  
}  

// Функция для сортировки таблицы
function applySort() {
    // Получаем выбранные значения сортировки
    const sortFIO = document.getElementById('sortFIO').value;
    const sortPosition = document.getElementById('sortPosition').value;
    const sortDepartment = document.getElementById('sortDepartment').value;
    const sortStatus = document.getElementById('sortStatus').value;
    const sortDate = document.getElementById('sortDate').value;

    // Получаем все строки таблицы, кроме первой строки
    let rows = Array.from(document.querySelectorAll('tbody tr')).slice(1);

    // Сортировка по ФИО
    if (sortFIO) {
        rows.sort((a, b) => {
            const fioA = a.querySelector('.column-fio').textContent.trim().toLowerCase();
            const fioB = b.querySelector('.column-fio').textContent.trim().toLowerCase();
            return sortFIO === 'asc' ? fioA.localeCompare(fioB) : fioB.localeCompare(fioA);
        });
    }
    ...
    if (sortPosition) {
        rows = rows.filter(row => {
            const position = row.querySelector('.column-position').textContent.trim().toLowerCase();
            return position === sortPosition.toLowerCase();
        });
    }
    ...
    if (sortDepartment) {
        rows = rows.filter(row => {
            const department = row.querySelector('.column-department').textContent.trim().toLowerCase();
            return department === sortDepartment.toLowerCase();
        });
    }
    ...
    if (sortStatus) {
        rows = rows.filter(row => {
            const status = row.querySelector('.column-status').textContent.trim().toLowerCase();
            return status === sortStatus.toLowerCase();
        });
    }
    ...
    if (sortDate) {
        rows.sort((a, b) => {
            const dateA = a.querySelector('.column-date').textContent.trim();
            const dateB = b.querySelector('.column-date').textContent.trim();
            const [dayA, monthA, yearA] = dateA.split('.').map(Number);
            const [dayB, monthB, yearB] = dateB.split('.').map(Number);
            const fullDateA = new Date(yearA, monthA - 1, dayA);
            const fullDateB = new Date(yearB, monthB - 1, dayB);
            return sortDate === 'new' ? fullDateB - fullDateA : fullDateA - fullDateB;
        });
    }

    // Очищаем таблицу и добавляем первую строку с инпутами обратно
    const tbody = document.querySelector('tbody');
    const firstRow = document.querySelector('tbody tr'); // Первая строка с инпутами
    tbody.innerHTML = '';
    tbody.appendChild(firstRow); // Вставляем первую строку снова
    rows.forEach(row => tbody.appendChild(row)); // Добавляем отсортированные строки

    // Закрываем модальное окно после применения сортировки
